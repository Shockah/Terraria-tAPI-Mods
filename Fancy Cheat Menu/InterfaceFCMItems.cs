...
namespace Shockah.FCM
{
	public class InterfaceFCMItems : InterfaceFCM<Item>
	{
		...
		protected readonly Filter<Item>
			FHead = new Filter<Item>("Head", Defs.items["Vanilla:Adamantite Helmet"].GetTexture(), (item) => { return item.headSlot != -1; }),
			FBody = new Filter<Item>("Body", Defs.items["Vanilla:Hallowed Plate Mail"].GetTexture(), (item) => { return item.bodySlot != -1; }),
			FLegs = new Filter<Item>("Legs", Defs.items["Vanilla:Titanium Leggings"].GetTexture(), (item) => { return item.legSlot != -1; }),
			FVanity = new Filter<Item>("Vanity", Defs.items["Vanilla:Clown Shirt"].GetTexture(), (item) => { return item.vanity; }),
			FMelee = new Filter<Item>("Melee", Defs.items["Vanilla:Terra Blade"].GetTexture(), (item) => { return item.damage > 0 && item.melee; }),
			FRanged = new Filter<Item>("Ranged", Defs.items["Vanilla:S.D.M.G."].GetTexture(), (item) => { return item.damage > 0 && item.ranged && (item.ammo == 0); }),
			FAmmo = new Filter<Item>("Ammo", Defs.items["Vanilla:Chlorophyte Bullet"].GetTexture(), (item) => { return item.damage > 0 && item.ranged && item.ammo != 0 && !item.notAmmo; }),
			FMagic = new Filter<Item>("Magic", Defs.items["Vanilla:Golden Shower"].GetTexture(), (item) => { return item.damage > 0 && item.magic; }),
			FSummon = new Filter<Item>("Summon", Defs.items["Vanilla:Pygmy Staff"].GetTexture(), (item) => { return item.damage > 0 && item.summon; }),
			FAccessory = new Filter<Item>("Accessory", Defs.items["Vanilla:Ankh Shield"].GetTexture(), (item) => { return item.accessory; }),
			FPickaxe = new Filter<Item>("Pickaxe", Defs.items["Vanilla:Picksaw"].GetTexture(), (item) => { return item.pick > 0; }),
			FAxe = new Filter<Item>("Axe", Defs.items["Vanilla:Spirit Hamaxe"].GetTexture(), (item) => { return item.axe > 0; }),
			FHammer = new Filter<Item>("Hammer", Defs.items["Vanilla:Pwnhammer"].GetTexture(), (item) => { return item.hammer > 0; }),
			FConsumable = new Filter<Item>("Consumable", Defs.items["Vanilla:Lesser Healing Potion"].GetTexture(), (item) => { return item.consumable && item.damage <= 0 && item.createTile == -1 && item.tileWand == -1 && item.createWall == -1 && item.ammo == 0 && item.name != "Xmas decorations"; }),
			FDye = new Filter<Item>("Dye", Defs.items["Vanilla:Flame Dye"].GetTexture(), (item) => { return item.dye != 0; }),
