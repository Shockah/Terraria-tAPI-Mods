			...
			if (item.mana > 0)
			{
				...
				if (item.type != 127 || !player.spaceGun)
				{
					...
					if ((int)itemMana > 0)
					{
						...
						if (style == ETipStyle.Vanilla) tip += new STooltip.Line(CText("Uses ", color, itemMana, (bool)options["itemManaCostTypeColor"].Value ? Color.DeepSkyBlue : Color.White, " mana"));
						if (style == ETipStyle.TwoCols) tip += new STooltip.Line(CText((bool)options["itemManaCostTypeColor"].Value ? Color.DeepSkyBlue : Color.White, "Mana#; cost:"), CText(color, itemMana));
					}
				}
			}
		}
